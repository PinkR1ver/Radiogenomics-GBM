...
if __name__ == '__main__':
    ...
    for iter_out in range(300):
        ...
        if epoch % 5 == 0:
            ...
            plt.title(f'test times {int(epoch/5)}: Test Loss')
            plt.savefig(os.path.join(predictPath, 'FLAIR', 'test_loss_monitor', f'test_times_{int(epoch/5)}_TestLoss_overview.png'))
            
            fig = plt.figure(num="Test_Sensitivity", figsize=(30, 30))
            sns.histplot(testSensitivityList, stat="probability", kde=True)
            plt.title(f'test times {int(epoch/5)}: Sensitivity')
            plt.savefig(os.path.join(predictPath, 'FLAIR', 'test_sensitivity_monitor', f'test_times_{int(epoch/5)}_Sensitivity_overview.png'))

            fig = plt.figure(num="Test_Specificity", figsize=(30, 30))
            plt.title(f'test times {int(epoch/5)}: Specificity')
            sns.histplot(testSpecificityList, stat="probability", kde=True)
            plt.savefig(os.path.join(predictPath, 'FLAIR', 'test_specificity_monitor', f'test_times_{int(epoch/5)}_Specificity_overview.png'))
        ...
        plt.title(f'epoch{epoch}: Train Loss')
        plt.savefig(os.path.join(savePath, 'FLAIR', 'train_loss_monitor', f'epoch{epoch}_TrainLoss_overview.png'))

        fig = plt.figure(num="Train_Sensitivity", figsize=(30, 30))
        sns.histplot(trainSensitivityList, stat="probability", kde=True)
        plt.title(f'epoch{epoch}: Sensitivity')
        plt.savefig(os.path.join(savePath, 'FLAIR', 'train_sensitivity_monitor', f'epoch{epoch}_Sensitivity_overview.png'))

        fig = plt.figure(num="Train_Specificity", figsize=(30, 30))
        sns.histplot(trainSpecificityList, stat="probability", kde=True)
        plt.title(f'epoch{epoch}: Specificity')
        plt.savefig(os.path.join(savePath, 'FLAIR', 'train_specificity_monitor', f'epoch{epoch}_Specificity_overview.png'))
