...
T1WeightPath = os.path.join(modelPath, 'T1_unet.pth')
...
if __name__ == '__main__':
    ...
    f = open(os.path.join(modelPath, 'T1_epoch.txt'), "r") 
    epoch = int(f.read())
    f.close()
    ...
    for iter_out in range(300):
        ...
        if epoch % 5 == 0:
            ...
            plt.title(f'test times {int(epoch/5)}: Test Loss')
            plt.savefig(os.path.join(predictPath, 'T1', 'test_loss_monitor', f'test_times_{int(epoch/5)}_TestLoss_overview.png'))
            
            fig = plt.figure(num="Test_Sensitivity", figsize=(30, 30))
            sns.histplot(testSensitivityList, stat="probability", kde=True)
            plt.title(f'test times {int(epoch/5)}: Sensitivity')
            plt.savefig(os.path.join(predictPath, 'T1', 'test_sensitivity_monitor', f'test_times_{int(epoch/5)}_Sensitivity_overview.png'))

            fig = plt.figure(num="Test_Specificity", figsize=(30, 30))
            plt.title(f'test times {int(epoch/5)}: Specificity')
            sns.histplot(testSpecificityList, stat="probability", kde=True)
            plt.savefig(os.path.join(predictPath, 'T1', 'test_specificity_monitor', f'test_times_{int(epoch/5)}_Specificity_overview.png'))
        ...
        plt.title(f'epoch{epoch}: Train Loss')
        plt.savefig(os.path.join(savePath, 'T1', 'train_loss_monitor', f'epoch{epoch}_TrainLoss_overview.png'))

        fig = plt.figure(num="Train_Sensitivity", figsize=(30, 30))
        sns.histplot(trainSensitivityList, stat="probability", kde=True)
        plt.title(f'epoch{epoch}: Sensitivity')
        plt.savefig(os.path.join(savePath, 'T1', 'train_sensitivity_monitor', f'epoch{epoch}_Sensitivity_overview.png'))

        fig = plt.figure(num="Train_Specificity", figsize=(30, 30))
        sns.histplot(trainSpecificityList, stat="probability", kde=True)
        plt.title(f'epoch{epoch}: Specificity')
        plt.savefig(os.path.join(savePath, 'T1', 'train_specificity_monitor', f'epoch{epoch}_Specificity_overview.png'))
